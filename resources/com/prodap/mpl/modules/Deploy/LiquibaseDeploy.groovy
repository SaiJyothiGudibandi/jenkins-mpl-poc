...
sh "docker run --rm -v ${volume}:/changelogs -e LIQUIBASE_HOST=${host} -e LIQUIBASE_USERNAME=${username} -e LIQUIBASE_PASSWORD=${password} -e LIQUIBASE_DATABASE=${db} -e LIQUIBASE_PORT=3306 ${liquibaseImagem} liquibase --changeLogFile=/changelogs/changelog.xml updateToTag ${tag}"